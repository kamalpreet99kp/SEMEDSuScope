...
#  - 6-8 (Sul-*) one color
#  - 9-11 (Gg-*) one color
#  - 12-14 (PbS/Pb-S-*) one color
#  - others separate colors
# ============================================================

# Choose easy-to-see pastel groups + distinct singles
COLOR_SUMMARY = "FFF200"   # yellow
COLOR_F       = "FF4D4D"   # red
COLOR_MIX     = "D9D9D9"   # grey
COLOR_RIM     = "FFD966"   # light orange/yellow

COLOR_CU  = "F8CBAD"       # Cu group (peach)
COLOR_SUL = "D9E1F2"       # Sul group (light blue)
COLOR_GG  = "C6EFCE"       # Gg group (light green)
COLOR_PBS = "E4DFEC"       # PbS group (light purple)

COLORS = {"Summary": COLOR_SUMMARY, "F": COLOR_F, "Mix": COLOR_MIX, "Rim": COLOR_RIM}
for h in ("Cu-A", "Cu-E", "Cu-L"):
    COLORS[h] = COLOR_CU
for h in ("Sul-A", "Sul-E", "Sul-L"):
    COLORS[h] = COLOR_SUL
for h in ("Gg-A", "Gg-E", "Gg-L"):
    COLORS[h] = COLOR_GG
for h in ("PbS-A", "PbS-E", "Pb-S-L"):
    COLORS[h] = COLOR_PBS
...
def choose_sheets_gui(sheet_names):
    win = tk.Toplevel()
    win.title("Select sheets to update (Ag categories)")
