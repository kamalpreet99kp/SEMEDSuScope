...
def main():
    ...
    try:
        ...
        selected_sheets = choose_sheets_listbox(root, wb.sheetnames)
        if not selected_sheets:
            messagebox.showwarning("No sheets selected", "No sheets were selected for processing.", parent=root)
            return
        ...
        for sheet_name in wb.sheetnames:
            ...
            ws = wb[sheet_name]
            key = canonical_key(sheet_name)
            cat_folder = folder_map.get(key)
            if cat_folder is None:
                skipped.append(f"{sheet_name}: no matching folder")
                continue
