...
- `run_au_word_report_macro.py` is the recommended launcher for Word export. It creates a temporary macro-enabled Word host document, imports `create_au_word_report_macro.bas`, and runs `CreateAuWordReportFromWorkbook` automatically so you do not need to import the macro manually every time.
...
`run_au_word_report_macro.py` is preferred for Word output because it imports and runs `create_au_word_report_macro.bas` automatically. This keeps the copy/paste inside Word VBA, closer to the manual Paste Special workflow used for the reference file, without manually importing the macro each time.
