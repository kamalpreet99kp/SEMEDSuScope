...
def bold_block_average_range(worksheet, row_number: int, first_column: int, reflected_light_column: int) -> None:
    """Bold the Average row cells before the image columns in an organized block."""
    last_average_column = first_column + reflected_light_column - 2
    if last_average_column < first_column:
        return
    average_range = worksheet.Range(worksheet.Cells(row_number, first_column), worksheet.Cells(row_number, last_average_column))
...
def reapply_average_bold_in_block(
    worksheet,
    output_row: int,
    source_start_row: int,
    source_last_row: int,
    source_count: int,
    first_output_column: int,
    reflected_light_column: int,
) -> None:
    """Reapply Average-row bolding after standard block formatting clears row bold."""
    source_end_row = source_start_row + source_count - 1
    if source_count <= 0 or not (source_start_row <= source_last_row <= source_end_row):
        return
    average_output_row = output_row + 1 + (source_last_row - source_start_row)
    bold_block_average_range(worksheet, average_output_row, first_output_column, reflected_light_column)
...
def organize_blocks(report_workbook, source_worksheet, layout: SampleFinishLayout, source_last_row: int) -> tuple:
    ...
    while source_start_row <= source_last_row:
        ...
        reapply_average_bold_in_block(block_sheet, output_row, source_start_row, source_last_row, left_count, 1, reflected_light_column)
        if right_count > 0:
            reapply_average_bold_in_block(
                block_sheet,
                output_row,
                right_start_row,
                source_last_row,
...
def main() -> None:
    ...
    finish_report_workbook(layout, report_path, data_path, output_path)

    messagebox.showinfo("Au report finished", f"Finished workbook saved successfully:\n\n{output_path}")


def finish_report_workbook(
    layout: SampleFinishLayout,
    report_path: Path,
    data_path: Path,
    output_path: Path,
) -> Path:
    """Finish an Au report workbook without interactive file prompts."""
    ...
    try:
        ...
        organize_blocks(report_workbook, report_worksheet, layout, final_data_row)
