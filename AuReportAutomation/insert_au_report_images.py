...
def build_intermediate_output_name(layout: SampleLayout, data_workbook_path: Path | None) -> str:
    """Build the default intermediate workbook name from the selected Excel file when available."""
    if data_workbook_path is not None:
        return f"{data_workbook_path.stem}_Inter.xlsx"
    return f"Au_Report_{layout.sample_type.replace('+', '_')}_Inter.xlsx"
...
def prepare_embedded_image(image_path: Path, temporary_folder: Path, image_role: str, row_number: int) -> Path:
    """Create a compressed embedded copy to keep the final workbook size practical.

    The original image file is never modified. The embedded copy is capped at 300 px
    on the longest side, which is far above the displayed 1.58 cm size in Excel but
    much smaller than raw SEM/camera images. This is close to Excel's own picture
    compression behavior and keeps output workbooks closer to manually produced files.
    The temporary filename includes the image role and row number so reflected-light
...
def exact_report_image_size() -> tuple[int, int]:
    """Return the report display size in Excel pixels."""
...
def add_image_to_cell(
    ...
    """Insert one compressed image copy into a worksheet cell at the report display size."""
    embedded_image_path = prepare_embedded_image(image_path, temporary_folder, image_role, row_number)
...
def create_image_workbook(
    ...
    set_standard_dimensions(worksheet, max_row=final_row, max_column=max_column)
    apply_standard_text_format(worksheet, max_row=final_row, max_column=max_column)
    ...
    for index in range(image_count):
        ...
        if index < len(reflected_images):
            add_image_to_cell(worksheet, reflected_images[index], row_number, layout.reflected_light_column, temporary_folder, "microscope")
        if index < len(sem_images):
            add_image_to_cell(worksheet, sem_images[index], row_number, layout.sem_column, temporary_folder, "sem")
...
def main() -> None:
    ...
    with TemporaryDirectory() as temporary_directory:
        ...
        output_file = choose_output_file(build_intermediate_output_name(layout, data_workbook_path))
